...
menucolors(void)
{
	/* Main tone is grayish, with negative selection */
	back = display->black;
	high = allocimage(display, Rect(0,0,1,1), CMAP8, 1, 0xFFD70BFF);	/* menu back */
	bord = allocimage(display, Rect(0,0,1,1), CMAP8, 1, 0x937C06FF);	/* menu border */
	text = allocimage(display, Rect(0,0,1,1), CMAP8, 1, 0xFFD70BFF);
...
menuscrollpaint(Image *m, Rectangle scrollr, int off, int nitem, int nitemdrawn)
{
	...
	if(menutxt == 0)
		menutxt = allocimage(display, Rect(0,0,1,1), CMAP8, 1, 0x222222FF);	/* border color; BUG? */
	if(menutxt)
